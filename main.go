...
func main() {
	...
	if len(args) > 0 {
		...
		if idxErr == nil && idx >= 0 && idx < len(sessions) {
			...
			if session != nil {
				session.Size = s.Size         // Carry over the size
				session.LastTime = s.LastTime // Carry over last time
			}
		} else {
			// Try as path
...
func parseGemini(path string) (*Session, error) {
	...
	seen := make(map[string]bool)
	// Extract project from path: .../tmp/<project>/chats/...
	...
	for scanner.Scan() {
		...
		if msgType == "user" {
			...
			// Sometimes content is empty if there are thoughts or tool calls
			if content == "" {
...
func parseClaude(path string) (*Session, error) {
	...
	seen := make(map[string]bool)
	// Extract project from path: .../projects/<project>/...
	parts := strings.Split(path, string(os.PathSeparator))
	for i, p := range parts {
